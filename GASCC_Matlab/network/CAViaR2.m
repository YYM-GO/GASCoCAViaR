function [BetaHat,standardErrors,VaR,Hit] = CAViaR2(MODEL,THETA,data)
...
inSampleObs = length(data);				    % Number of in sample observations.

 ytot        = data;
totalObs    = size(ytot,1);			% Number of total observations.
%y           = dataCAViaR(1:inSampleObs,:);   % Vector of in sample observations.

nSamples    = size(data,2);						  % Number of portfolio time series.


% *****************************************************************************************
% Set parameters for optimisation.
% *****************************************************************************************
REP			  = 100;                % Number of times the optimization algorithm is repeated.
if (MODEL == 1) | (MODEL == 3)
...
elseif MODEL == 2
    nInitialVectors = [30000, 4]; % Number of initial vector fed in the uniform random number generator for AS model.
    nInitialCond = 50;            % Select the number of initial conditions for the optimisation.
...
rand('seed', 100)                  % Set the random number generator seed for reproducability (seed used in the paper = 50).
...
%    empiricalQuantile = data(round(length(data)*THETA));
empiricalQuantile = quantile(data(1:500),THETA);
...
%**************************** Optimization Routine ******************************************  
    ...
    for i = 1:nInitialVectors(1)
        RQfval(i) = RQobjectiveFunction(initialTargetVectors(i,:), 1, MODEL, inSampleObs, data, THETA, empiricalQuantile);
    end
    ...
    for i = 1:size(BestInitialCond,1)
        [Beta(i,:), fval(i,1), exitflag(i,1)] = fminsearch('RQobjectiveFunction', BestInitialCond(i,:), options, 1, MODEL, inSampleObs, data(:,t), THETA, empiricalQuantile(t));
        for it = 1:REP
            [Beta(i,:), fval(i,1), exitflag(i,1)] = fminunc('RQobjectiveFunction', Beta(i,:), options, 1, MODEL, inSampleObs, data(:,t), THETA, empiricalQuantile(t));
            [Beta(i,:), fval(i,1), exitflag(i,1)] = fminsearch('RQobjectiveFunction', Beta(i,:), options, 1, MODEL, inSampleObs, data(:,t), THETA, empiricalQuantile(t));
            if exitflag(i,1) == 1
                break
            end
        end
...
%     for i = 1:size(Beta,1)
%         a = RQobjectiveFunction(Beta(i,:)', 2, MODEL, totalObs, ytot, THETA, empiricalQuantile);
%         VaRtest(:,i) = a(:,1);
%     end
...
%************************** Compute variables that enter output *****************************
    ...
    [varCov, D, gradient] = VarianceCovariance(BetaHat(:,t)', MODEL, inSampleObs, data(:, t), THETA, VaR(1:inSampleObs,t));
