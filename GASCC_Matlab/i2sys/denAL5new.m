function [f,gamma,delta] = denAL5new(z,gasp,gamma0,delta0,THETA2,Q,es0,p)
...
s = zeros(size(z,1),1);
for t = 1:size(z,1)
    delta(t) = max(THETA2*(es0-(1+exp(gamma(t)))*Q(t)),0.005);
    ...
    gamma(t+1) = gasp(1)+gasp(2)*s(t)+gasp(3)*gamma(t);
