...
% Fits TARCH models for use in DCC and related estimators (after some change to Kevin Sheppard's MFE tool box)
%
% USAGE:
...
%     [parameters, ~, ht, ~, ~, scores, diagnostics] = tarch(data(:,i),p(i),o(i),q(i), [], gjrType(i), volStartingVals, univariteOptions);
    [parameters, ~, ~, ~, ht] = multigarch_AJP(data(:,i),1,0,1,'GJRGARCH','NORMAL',[],[cov(data(:,i))*0.05;0.05;0.9]);
