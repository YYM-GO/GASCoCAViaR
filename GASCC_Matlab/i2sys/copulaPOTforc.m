function [copulaVaRPOT,copulaCoVaRPOT,copulaCoESPOT] = copulaPOTforc(xvec,yvec,xnew,ynew,THETA1,THETA2,KK,HESSgumbel,NN,RR,HESSstudt,RRnorm,HESSnorm,KKclayton,HESSclayton,tseed,iseed)
...
UhatOOSall(:,1) =  min(max(POTcdf(stdresidsOS(:,1),uL1,GARCHparams1IS(4),GARCHparams1IS(5)),1/(stepst)),(stepst-1)/(stepst));
UhatOOSall(:,2) =  min(max(POTcdf(stdresidsOS(:,2),uL2,GARCHparams2IS(4),GARCHparams2IS(5)),1/(stepst)),(stepst-1)/(stepst));
