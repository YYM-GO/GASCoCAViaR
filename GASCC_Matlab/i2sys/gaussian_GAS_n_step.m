function [rhot,ft] = gaussian_GAS_n_step(theta,lag_rho,Umat,HESSnorm,stepst)
...
It = interp1(HESSnorm(:,1),HESSnorm(:,2),rhot(i-1));
DELTAt = (-NormalCopula_CL(rhot(i-1)+h,Umat(i-1,:))--NormalCopula_CL(rhot(i-1),Umat(i-1,:)))/h    ;         % estimated score for rho. NOTE: my tcopulaCL2 code returns the *neg* log-like, so need to undo that here
