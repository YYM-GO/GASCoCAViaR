function [paramestall,CoVaRall,CoESall,gammaall,deltaall,Scoreall] = GASGARCH1sys2inew(dataX,dataY,VaRsector,THETA1,THETA2,CoMODEL,p,epsil,miter)
...
parfor i = 1:N
    if CoMODEL == 1
        [cobetaSAV(:,i),~,CoVaRSAV(:,i)] = optcavsav27(THETA2,dataX,dataY(:,i),VaRsector); warning off
    else
        [cobetaAS(:,i),~,CoVaRAS(:,i)] = optcavAS28(THETA2,dataX,dataY(:,i),VaRsector); warning off
    end
end
...
parfor i = 1:N
    ...
    es = mean(dataY(find(dataX<-VaRsector),i));
    gamma00 = gamma0(:,i);
    delta00 = delta0(:,i);
    CoVEM = CoVaREM(:,i);
    ...
    while dif>epsil && iter<miter
        Lc = Lcnew;
        [f,gamma,delta] = denAL2new(dataY(:,i),paraminit(1:3),gamma00,delta00,THETA2,-CoVEM,es,p);
        
        % EM (E step) for AL density
        
        [ut,zt] = uandz(dataY(:,i),-CoVEM,delta,THETA2);
        
        %%% M-step
        txlv = find(dataX<-VaRsector); %find the time that X < VaR(X)
        opt3 = @(params)ElcALGAS5new(params,dataY(:,i),zt,dataX,-VaRsector,es,THETA1,THETA2,p,CoMODEL,gamma0(:,i),delta0(:,i),txlv);
        [paramest2,fval,exitflag]  = fmincon(opt3,paraminit,A,1,...
            [],[],[-0.2;-0.2;0.75;max(paraminit(4:end)-abs(paraminit(4:end))*0.3,paraminit0(4:end)-abs(paraminit0(4:end))*0.3)],...
        ...
        [ElAL,~,~,CoVEMnew,CoESEMnew,scoreal] = ElcALGAS5new(paraminit,dataY(:,i),zt,dataX,-VaRsector,es,THETA1,THETA2,p,CoMODEL,gamma0(:,i),delta0(:,i),txlv);
        CoVEM = -CoVEMnew;
        CoESEM = -CoESEMnew;
        Lcnew = ElAL;
        paraminit = paramest2
