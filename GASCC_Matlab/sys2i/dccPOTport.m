function [parametersall, ll ,Ht, Qt, Rt]=dccPOTport(data,dataAsym,m,l,n,p,o,q,gjrType,method,composite,startingVals,options)
% Estimation of scalar DCC(m,n) by the help of the dcc function in Kevin Sheppard's MFE tool box (after some change)
% we change the marginal distribution of the DCC MODEL to POT model, 
% while the joint distribution is still bivariate gaussian distribution
% 
%
% USAGE:
...
%
% COMMENTS:
%   the 1 to (end-3) parameters are marginal parameters, the last three parameters are DCC parameters.
% Example: for the bivaraite DCC-GARCH(1,1) case, the parameters(1:6) and parameters (7:12) are the marginal parameters for the two margins by using the POT distribution fitting.
% the first 3 parameter of the 6 marginal parameters are GARCH parameters,
% the fourth to sixth parameters are the beta \xi and uL parameters of POT distribution.
...
[H,univariate,parameters,parametersm] = dcc_fit_variance_POT2(data2d,p,o,q,gjrType,tarchStartingVals);
stdData = data;
...
isJoint = false;
isInference = false;
% [ll,~,Rt,Qt] = dcc_likelihood(parameters,data,dataAsym,m,l,n,[],[],backCast,backCastAsym,stage,composite,isJoint,isInference,gScale,univariate);
[ll,~,Rt,Qt] = dcc_likelihood(parameters,data,dataAsym,m,l,n,R,N,backCast,backCastAsym,stage,composite,isJoint,isInference,gScale,univariate);
ll = -ll;
Ht = zeros(k,k,T);
for t=1:T
    h = sqrt(H(t,:));
...
%     VCV = Ainv*B*Ainv'/T;
% end
parametersall = [reshape(parametersm,1,[]),parameters(end-2:end)];
