function [DCCCoVaR,DCCCoES] = dccGauportforc(xvec,yvec,xnew,ynew,THETA1,THETA2,tforc)
...
for k =2:(tforc+1)
    ...
    DCCCoVaR0(k) = CoVaRfrocNormalN(THETA1,THETA2,Rtnew(1,2,k),diag(Htnew(:,:,k)),ones(tforc+1,1),MUhat);
    grid = [0.005:0.01:THETA2];
    for l = 1:length(grid)
        esgrid(l) = CoVaRfrocNormalN(THETA1,grid(l),Rtnew(1,2,k),diag(Htnew(:,:,k)),ones(tforc+1,1),MUhat); warning off
    end
