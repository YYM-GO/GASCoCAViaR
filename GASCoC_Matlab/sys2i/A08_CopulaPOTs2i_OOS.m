...
CopulaVaRs2iPOTOOS = nan(Tout,N,4);
CopulaCoVaRs2iPOTOOS = nan(Tout,N,4);
CopulaCoESs2iPOTOOS = nan(Tout,N,4);
Copulaconvars2iPOTOOS = nan(Tout,N);
Copulacorrs2iPOTOOS = nan(N,N,Tout);
for t = 1:nstepst
    tic
    if t ~= nstepst
        stepst = stepst0;
    ...
    parfor i = 1:N
        ...
        [copulaVaR,copulaCoVaR,copulaCoES,condvar2out,stdresidsIS2] = copulaPOTforc(xvec,yvec,xnew,ynew,THETA1,THETA2,KK,HESSgumbel,NN,RR,HESSstudt,RRnorm,HESSnorm,KKclayton,HESSclayton,t,i);
    ...
    if t ~= nstepst
        CopulaVaRs2iPOTOOS(stepst*(t-1)+1:stepst*t,:,:) = copulaVaRall;
        CopulaCoVaRs2iPOTOOS(stepst*(t-1)+1:stepst*t,:,:) = copulaCoVaRall;
        CopulaCoESs2iPOTOOS(stepst*(t-1)+1:stepst*t,:,:) = copulaCoESall;
        Copulaconvars2iPOTOOS(stepst*(t-1)+1:stepst*t,:) = convarall;
        Copulacorrs2iPOTOOS(:,:,stepst*(t-1)+1:stepst*t) = repmat(corr(stdresidsall),1,1,stepst);
    else
        CopulaVaRs2iPOTOOS(end-stepst+1:end,:,:) = copulaVaRall;
        CopulaCoVaRs2iPOTOOS(end-stepst+1:end,:,:) = copulaCoVaRall;
        CopulaCoESs2iPOTOOS(end-stepst+1:end,:,:) = copulaCoESall;
        Copulaconvars2iPOTOOS(end-stepst+1:end,:) = convarall;
        Copulacorrs2iPOTOOS(:,:,end-stepst+1:end) = repmat(corr(stdresidsall),1,1,stepst);
    end
    [t,toc]
end

save CopulaVaRs2iPOTOOS.mat CopulaVaRs2iPOTOOS
save CopulaCoVaRs2iPOTOOS.mat CopulaCoVaRs2iPOTOOS
save CopulaCoESs2iPOTOOS.mat CopulaCoESs2iPOTOOS
save Copulaconvars2iPOTOOS.mat Copulaconvars2iPOTOOS
save Copulacorrs2iPOTOOS.mat Copulacorrs2iPOTOOS
