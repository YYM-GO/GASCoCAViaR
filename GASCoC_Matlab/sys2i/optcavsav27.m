...
for i = 1:size(BestInitialCond,1)
    ...
    [Beta(i,:), fval(i,1), exitflag(i,1)] = fmincon(flike,BestInitialCond(i,:),[],[],[],[],[-0.3*ones(1,3),0.7],[ones(1,4)],[],options); warning off
    for it = 1:REP
        [Beta(i,:), fval(i,1), exitflag(i,1)] = fminunc(flike,Beta(i,:),options);warning off
        if Beta(i,end)<0.7
            [Beta(i,:), fval(i,1), exitflag(i,1)] = fmincon(flike,BestInitialCond(i,:),[],[],[],[],[-0.3*ones(1,3),0.7],[ones(1,6)],[],options);warning off
        end
        if exitflag(i,1) == 1
            break
        end
