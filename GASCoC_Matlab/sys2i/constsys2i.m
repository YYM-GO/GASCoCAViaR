function [paramestall,CoVaRall,CoESall] = constsys2i(dataX,dataY,VaRsector,THETA1,THETA2,CoMODEL,epsil,miter)
...
parfor i = 1:N
    ...
    while dif>epsil && iter<miter
        Lc = Lcnew;
        [f,gamma,delta] = denALconst(dataY(:,i),gamma00,THETA2,-CoVEM,es);
        % EM (E step) for AL density
        [ut,zt] = uandz(dataY(:,i),-CoVEM,delta,THETA2);
        %%% M-step
        txlv = find(dataX<-VaRsector); %find the time that X < VaR(X)
        opt3 = @(params)ElcALGAS5const(params,dataY(:,i),zt,dataX,-VaRsector,es,THETA1,THETA2,CoMODEL,gamma0(:,i),txlv);
        ...
        [ElAL,~,~,CoVEMnew,CoESEMnew] = ElcALGAS5const(paraminit,dataY(:,i),zt,dataX,-VaRsector,es,THETA1,THETA2,CoMODEL,gamma0(:,i),txlv);
        CoVEM = -CoVEMnew;
        CoESEM = -CoESEMnew;
        Lcnew = ElAL;
        paraminit = paramest2;
