...
CopulaVaRi2sPOTOOS = nan(Tout,N,4);
CopulaCoVaRi2sPOTOOS = nan(Tout,N,4);
CopulaCoESi2sPOTOOS = nan(Tout,N,4);
for t = 1:nstepst
    tic
    if t ~= nstepst
        stepst = stepst0;
    ...
    parfor i = 1:N
        ...
        [copulaVaR,copulaCoVaR,copulaCoES] = copulaPOTforc(xvec,yvec,xnew,ynew,THETA1,THETA2,KK,HESSgumbel,NN,RR,HESSstudt,RRnorm,HESSnorm,KKclayton,HESSclayton,t,i);
        copulaVaRall(:,i,:) = copulaVaR;
        copulaCoVaRall(:,i,:) = copulaCoVaR;
        copulaCoESall(:,i,:) = copulaCoES;
    end
    if t ~= nstepst
        CopulaVaRi2sPOTOOS(stepst*(t-1)+1:stepst*t,:,:) = copulaVaRall;
        CopulaCoVaRi2sPOTOOS(stepst*(t-1)+1:stepst*t,:,:) = copulaCoVaRall;
        CopulaCoESi2sPOTOOS(stepst*(t-1)+1:stepst*t,:,:) = copulaCoESall;
    else
        CopulaVaRi2sPOTOOS(end-stepst+1:end,:,:) = copulaVaRall;
        CopulaCoVaRi2sPOTOOS(end-stepst+1:end,:,:) = copulaCoVaRall;
        CopulaCoESi2sPOTOOS(end-stepst+1:end,:,:) = copulaCoESall;
    end
    [t,toc]
end

save CopulaVaRi2sPOTOOS.mat CopulaVaRi2sPOTOOS
save CopulaCoVaRi2sPOTOOS.mat CopulaCoVaRi2sPOTOOS
save CopulaCoESi2sPOTOOS.mat CopulaCoESi2sPOTOOS