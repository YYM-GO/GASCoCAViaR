...
copulaVaRall = []; copulaCoVaRall = [];copulaCoESall = [];
CopulaVaRi2sGauOOS = nan(Tout,N,4);
CopulaCoVaRi2sGauOOS = nan(Tout,N,4);
CopulaCoESi2sGauOOS = nan(Tout,N,4);
...
for t = 1:nstepst
    ...
    parfor i = 1:N
        ...
        [copulaVaR,copulaCoVaR,copulaCoES] = copulaGauforc(xvec,yvec,xnew,ynew,THETA1,THETA2,KK,HESSgumbel,NN,RR,HESSstudt,RRnorm,HESSnorm,KKclayton,HESSclayton,t,i);
        copulaVaRall(:,i,:) = copulaVaR;
        copulaCoVaRall(:,i,:) = copulaCoVaR;
        copulaCoESall(:,i,:) = copulaCoES;
    end
    if t ~= nstepst
        CopulaVaRi2sGauOOS(stepst*(t-1)+1:stepst*t,:,:) = copulaVaRall;
        CopulaCoVaRi2sGauOOS(stepst*(t-1)+1:stepst*t,:,:) = copulaCoVaRall;
        CopulaCoESi2sGauOOS(stepst*(t-1)+1:stepst*t,:,:) = copulaCoESall;
    else
        CopulaVaRi2sGauOOS(end-stepst+1:end,:,:) = copulaVaRall;
        CopulaCoVaRi2sGauOOS(end-stepst+1:end,:,:) = copulaCoVaRall;
        CopulaCoESi2sGauOOS(end-stepst+1:end,:,:) = copulaCoESall;
    end
    [t,toc]
end

save CopulaVaRi2sGauOOS.mat CopulaVaRi2sGauOOS
save CopulaCoVaRi2sGauOOS.mat CopulaCoVaRi2sGauOOS
save CopulaCoESi2sGauOOS.mat CopulaCoESi2sGauOOS