...
CopulaVaRi2sGaufull = nan(T,N);
CopulaESi2sGaufull = nan(T,N);
CopulaCoVaRi2sGaufull = nan(T,N);
CopulaCoESi2sGaufull = nan(T,N);
...
parfor i = 1:N
    ...
    GARCHparams1 = zeros(1,3);
    GARCHparams2 = zeros(1,3);
    xvec = shenwan(:,i);
    yvec = hushen;
    resids = [xvec,yvec];
    MUhat = [0,0];
    for k=1:2
        %  GARCH(1,1)
        [parameters3, likelihood1, ~, ~, hhat1] = multigarch_AJP(resids(:,k),1,0,1,'GJRGARCH','NORMAL',[],[cov(resids(:,k))*0.05;0.05;0.9]);
        if k==1
    ...
    [GARCHparams1IS, ~, ~, ~, hhat1IS] = multigarch_AJP(resids(:,1),1,0,1,'GJRGARCH','NORMAL',[],GARCHparams1);
    [GARCHparams2IS, ~, ~, ~, hhat2IS] = multigarch_AJP(resids(:,2),1,0,1,'GJRGARCH','NORMAL',[],GARCHparams2);
    ...
    v=[hhat1IS,hhat2IS];
    for k = 1:T
        VaRforcgarch(k,:) = repmat(MUhat(1) + sqrt(v(k,1))*norminv(THETA1),1,4);
        ESforcgarch(k,:) = repmat(MUhat(1) - sqrt(v(k,1))*normpdf(norminv(THETA1))/(THETA1),1,4);
    end
    e(:,i,:) = VaRforcgarch;
    f(:,i,:) = ESforcgarch;
...
CopulaVaRi2sGaufull = e;
CopulaESi2sGaufull = f;
CopulaCoVaRi2sGaufull = o;
CopulaCoESi2sGaufull = m;

save CopulaVaRi2sGaufull.mat CopulaVaRi2sGaufull
save CopulaCoVaRi2sGaufull.mat CopulaCoVaRi2sGaufull
save CopulaCoESi2sGaufull.mat CopulaCoESi2sGaufull