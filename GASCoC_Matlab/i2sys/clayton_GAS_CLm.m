function [CL,kappat,ft] = clayton_GAS_CLm(theta,data,kappa0,hessINFO)
...
%         if abs(kappat(tt-1))<=0.00001    % 19oct04: Clayton cop is well defined at theta=0, but it is a limit. setting theta to zero in the following
%             kappat(tt-1) = 0.00001;     % formula involves 1/0. so just use theta=0.00001 for theta at or near zero.
%         end
...
%          k = max(min(-1+exp(ft(tt)),20),0.0001);
          k = -1+exp(ft(tt));
%         if abs(kappat(tt-1))<=0.00001    % 19oct04: Clayton cop is well defined at theta=0, but it is a limit. setting theta to zero in the following
%             kappat(tt-1) = 0.00001;     % formula involves 1/0. so just use theta=0.00001 for theta at or near zero.
%         end
...
CL = CL + (2+1./kappat).*log( max((data(:,1).^(-kappat)) + (data(:,2).^(-kappat)) -1,0.0001));
% CL = CL + (2+1./kappat).*log( (data(:,1).^(-kappat(tt-1)-h)) + (data(:,2).^(-kappat(tt-1)-h)) -1)
CL = log(1+kappat) - CL;
CL = sum(CL);
CL = -CL;
